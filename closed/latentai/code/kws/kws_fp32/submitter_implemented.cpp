...
string model_path = "bin/kws_fp32.so";
...
void th_load_tensor() {
  float input_pixels[kKwsInputSize];
  float input_asfloat[kKwsInputSize];

  size_t bytes = ee_get_buffer(reinterpret_cast<uint8_t *>(input_pixels),
                               kKwsInputSize * sizeof(float));
               
  if (bytes / sizeof(float) != kKwsInputSize) {
