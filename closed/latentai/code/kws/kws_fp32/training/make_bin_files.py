...
if __name__ == '__main__':
  ...
  # Load in compiled binary from LEIP SDK.
  lib = tvm.runtime.load_module('kws_fp32.so')
  ...
  if Flags.feature_type == "mfcc":
    ...
    # TFLite runtime is not being used. Commented out above and replaced with below.
  ...
  # Not needed for fp32 generation as input scale and zero point are 0.  
  ...
  for dat, label in eval_data:
    ...
    with open(os.path.join(test_file_path, fname), "wb") as fpo:
      # Write out `dat` instead of `dat_q` for unquantized fp32 dataset
      # fpo.write(dat_q.flatten())
      fpo.write(dat.flatten())

    if test_tfl_on_bin_files:
      # interpreter.set_tensor(input_details[0]['index'], dat_q)
      # interpreter.invoke()

      # Call inference for LEIP SDK binary.
      module.set_input('input_1', tvm.nd.array(dat))
      module.run()
      ...
      output_data.append(np.argmax(module.get_output(0).asnumpy()))
