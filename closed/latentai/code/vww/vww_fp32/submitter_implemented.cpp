...
string model_path = "/home/pi/mlperf_submissions/v0.1/reference_submissions/visual_wakewords_fp32/bin/vww_fp32.so";
...
void th_load_tensor() {
  uint8_t input_pixels[kVwwInputSize];
  float input_asfloat[kVwwInputSize];
  ...
  for(int i=0; i<kVwwInputSize;i++) {
	    input_asfloat[i] = ((float)input_pixels[i]/255.0);
  }

  model.createNDArray(input_asfloat);
  model.setInput();
}
